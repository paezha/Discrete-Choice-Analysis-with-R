...
\begin{array}{llllllll}
    ...
    V_{i3}-V_{i1}= & (\mu_3 - 0) & + &\beta_{11}(x_{i2} - x_{i1}) & + &\beta_{12}(z_ix_{i3} - z_ix_{i1}) & + &(\delta_3w_{i3} -  \delta_1w_{i1})\\
    V_{i3}-V_{i2}= & (\mu_3- \mu_2) & + &\beta_{11}(x_{i2} - x_{i1}) & + &\beta_{12}(z_ix_{i3} - z_ix_{i2}) & + &(\delta_3w_{i3} - \delta_2w_{i2})\\
\end{array}
$$
...
The function `model.matrix` allows us to see how the formula is applied to the data (we first convert the model matrix into a dataframe and then use the 8 function to display the top rows):
```{r}
...
Next, the likelihood function for this toy sample can be written as a function of $\mu$ and $\beta$. In this way, it is possible to calculate an initial value of the likelihood function bu setting $\mu$ and $\beta$ to zero. We will call this "Experiment 1":
```{r}
mu <- 0
beta <- 0
